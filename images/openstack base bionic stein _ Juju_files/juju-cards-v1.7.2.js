...
let jujuCards = demoDomain => {
  ...
  let renderCharm = (card, data) => {
    ...
    let dom = `<div class="juju-card__container charm-card">` + `<a href="${detailsLink}" class="charm-card__link">View details</a>` + `<header class="charm-card__header">` + `<img src="${image}" alt="${name}" class="charm-card__image" />` + `<h1 class="charm-card__title">${name}</h1>` + `<ul class="charm-card__meta">` + `<li class="charm-card__meta-item--by">by <a href="${ownerLink}">${owner}</a></li>` + `${seriesEle}` + `</ul>` + `</header>` + `<main class="charm-card__main">` + `<ul class="charm-card__actions">` + `<li class="charm-card__actions-item--details">` + `<label class="charm-card__actions-label" for="cli-deploy">Deploy with the CLI:</label>` + `<div class="charm-card__actions-command">` + `<input class="charm-card__actions-field js-clipboard-input" readonly="readonly" value="juju deploy ${id}" onclick="this.select();" id="cli-deploy">` + `<button class="charm-card__actions-copy-to-clipboard js-copy-to-clipboard" onclick="jujuCards.copyToClipboard(this);" title="Copy to clipboard">${copyIcon}</button>` + `</div>` + `</li>` + `<li class="charm-card__actions-item--demo">` + `<a href="${addLink}" class="charm-card__add-button--primary">${deployTitle}</a>` + `</li>` + `</ul>` + `</main>` + `<footer class="charm-card__footer">` + `<a href="http://jujucharms.com"><img src="https://assets.ubuntu.com/v1/7e21b535-logo-juju.svg" alt="" class="charm-card__footer-logo" /></a>` + `<p class="charm-card__footer-note">&copy; <a href="http://www.canonical.com">Canonical Ltd</a>.</p>` + `</footer>` + `</div>`;
